    ...
    const handleExport = async () => {
        try {
            ...
            const exportData = students.map(s => ({
                ...
                'Phone Number': s.phone || s.phoneNumber || '-',
                'Email': s.email || '-',
                'Semester': `Sem ${s.currentSemester || batch.currentSemester}`,
                'Academic Status': s.academicStatus || 'ACTIVE',
                'Backlog Count': (s.backlogSubjects || []).length,
                'Backlog Subjects': (s.backlogSubjects || []).join(', ') || 'None'
            }));
                ...
                <div className="fixed inset-0 z-[1200] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden">
                    ...
                    >
                        ...
                        <div className="p-8 sm:p-10 border-b border-gray-50 flex justify-between items-start sm:items-center gap-6 bg-gray-50/30">
                            <div>
                                <h2 className="text-3xl font-black text-gray-900 tracking-tight leading-none">Student List</h2>
                                <p className="text-[10px] font-black text-[#E31E24] uppercase tracking-widest mt-2 flex items-center gap-2">
                        ...
                        {/* Table */}
                        <div className="flex-1 overflow-x-auto p-0 overscroll-contain custom-scrollbar scrollbar-thin scrollbar-thumb-gray-200">
                            <table className="w-full min-w-[900px] text-left text-sm border-separate border-spacing-0">
                                <thead className="bg-white sticky top-0 z-10">
                                    <tr>
                                        <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Roll Number</th>
                                        <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Student Name</th>
                                        ...
                                        <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50 text-center">Semester</th>
                                        <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50 text-center">Academic Status</th>
                                        <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50 text-center">Status</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-50">
                                    {loading ? (
                                        <tr><td colSpan={6} className="p-24 text-center">
                                            ...
                                            <p className="mt-4 text-xs font-black text-gray-400 uppercase tracking-widest">Loading Students...</p>
                                        </td></tr>
                                    ) : students.length === 0 ? (
                                        <tr><td colSpan={6} className="p-24 text-center">
                                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest">No students found in this class.</p>
                                        </td></tr>
                                    ) : (
                                                ...
                                                <tr key={student.id} className={`group hover:bg-gray-50/50 transition-all ${
                                                    ...
                                                    <td className="px-8 py-6 text-center">
                                                        <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-tight ${
                                                            ...
                                                            {student.status?.toUpperCase() || 'OFFLINE'}
                                                        </span>
                                                    </td>
                                                </tr>
                                            );
                        ...
                        <div className="p-8 border-t border-gray-50 bg-gray-50/10 flex justify-end">
                            <button onClick={onClose} className="px-10 py-4 bg-white border border-gray-200 rounded-2xl font-black text-[10px] text-gray-400 hover:text-gray-900 uppercase tracking-[0.2em] transition-all shadow-sm active:scale-95">Close</button>
                        </div>
                    </motion.div>
                </div>
            )}
