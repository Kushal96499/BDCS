            ...
            <aside className="hidden md:flex fixed inset-y-0 left-0 xl:w-64 md:w-20 lg:w-20 flex-col z-50 transition-all duration-300">
                <div className="h-full bg-white/80 backdrop-blur-2xl border-r border-gray-100 flex flex-col py-8 shadow-[10px_0_40px_rgba(0,0,0,0.02)]">
                        ...
                        {NAV_ITEMS.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className={`text-sm font-bold tracking-tight hidden xl:block ${active ? 'font-black text-blue-700' : ''}`}>
                                        {item.name}
                                    </span>
                    ...
                    <div className="px-3 xl:px-4 mt-auto pt-6 border-t border-gray-50 space-y-4 flex flex-col items-center xl:items-start">
                        ...
                        >
                            <div className="w-10 h-10 rounded-xl overflow-hidden border border-white shadow-sm bg-gray-50 flex items-center justify-center text-blue-600 font-black text-xs shrink-0 ring-2 ring-transparent transition-all uppercase">
                                {user?.name?.[0] || 'P'}
                            </div>
                ...
                <div className="px-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[7px] font-bold text-blue-600 uppercase tracking-widest opacity-60">Principal Portal</p>
                        </div>
                    </div>

                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center font-black text-[10px] border border-blue-100">
                            {user?.name?.[0]}
                        </div>
                    </div>
                </div>
                    ...
                    {NAV_ITEMS.filter(i => i.showInMobileDock).map((item) => {
                        ...
                        return (
                            <button
                                key={item.path}
                                onClick={() => handleNavigate(item.path)}
                                className={`flex flex-col items-center gap-1.5 transition-all duration-300 ${active ? 'text-blue-600' : 'text-gray-400'}`}
                            >
                        ...
                        <>
                            ...
                            >
                                <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center p-1 border border-blue-100">
                                            <img src="/assets/biyani-logo.png" className="w-full h-full object-contain" alt="" />
                                        </div>
                                        <h3 className="text-[10px] font-black text-gray-900 uppercase tracking-widest">Principal Menu</h3>
                                    </div>
                                ...
                                <div className="p-8 border-t border-gray-50 bg-gray-50/30 space-y-4">
                                     <div className="flex items-center gap-3 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
                                        <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 font-black uppercase text-sm border border-blue-100">
                                            {user?.name?.[0]}
                                        </div>
                                        <div>
                                            <p className="text-[11px] font-black text-gray-900 tracking-tight">{user?.name}</p>
                                            <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Institutional Principal</p>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => auth.signOut().then(() => navigate('/login'))}
